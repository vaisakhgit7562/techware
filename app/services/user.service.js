...
class UserService {
  ...
  static async fetchAll(type, limit, offset, page) {
    ...
    const resp = await database.users.findAndCountAll({ where: condition, limit, offset });
  ...
  static async fetch_dashboard(type, limit, offset, page,user_id) {
    ...
    const user = await database.users.findOne({  raw: true,where: { user_id: user_id } });
    const paid = await database.paid.findOne({  raw: true,where: { user_id: user_id } });
    var m_data = [];    
    ...
    if(paid){ 
    if(paid.paymentChittyId==res.paymentChittyId && user.user_id==paid.user_id){  
    return; 
    }
    }
    m_data.push(res);  
  ...
  static async fetchOne(userId) {
    const resp = await database.users.findByPk(userId);
    return resp;
  }

  static async getUserId(userId) {
    const resp = await database.users.findOne({ where: { usersId: userId } });
    return resp;
  }

  static async updateData(userId, postData) {
    const resp = await database.users.update(postData, { where: { id: userId } });
    return resp;
  }

  static async deactive(userId) {
